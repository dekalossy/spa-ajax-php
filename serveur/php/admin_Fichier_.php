...
if(isset($_GET['nom']) AND isset($_GET['courriel'])){   // ETAPE 1

		$getNom = $_GET['nom'];
		$getcourriel = $_GET['courriel'];

							//stockage dans $admin

										// 1- VERIFICATION DE L'EXISTENCE DE VARIABLES DE SESSION

		 if(isset($_SESSION['nom']) AND $getNom == $_SESSION['nom']  AND $getcourriel == $_SESSION['courriel'] AND $_SESSION['courriel']=='admin@amtfilm.com'){  // ETAPE 2
...
/******************************************
		 ...
		 **************************************************************************
		  ...
		  if(isset($_POST['newFilmEnreg'])){
		  	...
		  	$url = 'preview/'.$titreFilm;

		  	$enregistrementFilms = $bdd->prepare('INSERT INTO films (categorie, titre, realisateur, duree, dateFilm, pochette, url ) VALUES (?, ?, ?, ?, ?, ?, ?)');
		  	$enreg = $enregistrementFilms->execute(array($categFilm, $titreFilm, $realisateurFilm, $dureeFilm, $dateFilm, $pochette, $url));
		...
		/**************************************************************************/
		 	...
		 	echo '<p style="color:#fff"> Bienvenue '.$_SESSION['courriel'].'</p><br>'
							   		...
							   		<div id="enregFilms">
										<form id="formEnregFilms" class="formFilms" action="" method="POST"   enctype="multipart/form-data">
											...
											<div class="form-row d-flex">
												...
												<input type="text" class="form-control" name="categFilm" id="categFilm" required>
							  					</div>	  
					  						 </div>
