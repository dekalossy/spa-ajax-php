		...
		$admin=$requser->fetch();							//stockage dans $admin
		 ...
		 /**************************************************************************/
		  ...
		  if(isset($_POST['newFilmEnreg'])){
		  	...
		  	$enregistrementFilms = $bdd->prepare('INSERT INTO films (categorie, titre, realisateur, duree, dateFilm, pochette, url, synopsys) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
		  	$enreg = $enregistrementFilms->execute(array($categFilm, $titreFilm, $realisateurFilm, $dureeFilm, $dateFilm, $pochette, $url, $synopsys));
